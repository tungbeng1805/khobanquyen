...
$wrap_attrs = [
    'id'               => $html_id,
    'class'            => trim('pxl-grid pxl-post-list ' . ($settings['scroll_effect'] != 'none' ? 'pxl-check-scroll ' : '') . 'pxl-effect--3d ' . esc_attr($settings['scroll_effect']) . ' layout-' . $layout),
...
if( count($posts) <= 0){
    echo '<div class="pxl-no-post-list">'.esc_html__( 'No Post Found', 'maiko' ). '</div>';
    return;
}
?>
...
<div <?php pxl_print_html($widget->get_render_attribute_string( 'wrapper' )) ?>>
    ...
    <?php endif; ?>
    <div class="pxl-list-inner d-flex-wrap relative row"> 
