...
$layout               = $widget->get_setting('layout_'.$post_type, 'portfolio-list-1');
...
$load_more = array(
    ...
    'img_size'            => $widget->get_setting('img_size','630x630'),
...
$wrap_attrs = [
    'id'               => $html_id,
    'class'            => trim('pxl-grid pxl-portfolio-list ' . ($settings['scroll_effect'] != 'none' ? 'pxl-check-scroll ' : '') . 'pxl-effect--3d ' . esc_attr($settings['scroll_effect']) . ' layout-' . $layout),
...
if( count($posts) <= 0){
    echo '<div class="pxl-no-portfolio-list">'.esc_html__( 'No Post Found', 'maiko' ). '</div>';
    return;
}
?>
...
<div <?php pxl_print_html($widget->get_render_attribute_string( 'wrapper' )) ?>>
    <?php if($settings['show_toolbar'] == 'show'): ?>
        <div class="portfolio-list-toolbar row align-items-center justify-content-between">
