 ...
 */
add_action('after_setup_theme', 'maiko_setup');
function maiko_setup(){

    //Set the content width in pixels, based on the theme's design and stylesheet.
    $GLOBALS['content_width'] = apply_filters( 'maiko_content_width', 1200 );

    // Make theme available for translation.
    load_theme_textdomain( 'maiko', get_template_directory() . '/languages' );
    ...
    register_nav_menus( array(
        'primary' => esc_html__( 'Primary', 'maiko' ),
    ) );
    ...
    // Enable support for Post Thumbnails on posts and pages.
    add_theme_support('post-thumbnails');
    add_image_size( 'maiko-thumb-small', 80, 70, true );
    add_image_size( 'maiko-thumb-xs', 120, 104, true );
    add_image_size( 'maiko-large', 810, 385, true );
    add_image_size( 'maiko-portfolio', 600, 600, true );
 ...
 */
add_action( 'widgets_init', 'maiko_widgets_position' );
function maiko_widgets_position() {
    register_sidebar( array(
        'name'          => esc_html__( 'Blog Sidebar', 'maiko' ),
        'id'            => 'sidebar-blog',
    ...
    if (class_exists('ReduxFramework')) {
        register_sidebar( array(
            'name'          => esc_html__( 'Page Sidebar', 'maiko' ),
            'id'            => 'sidebar-page',
            'before_widget' => '<section id="%1$s" class="widget %2$s">',
    ...
    if ( class_exists( 'Woocommerce' ) ) {
        register_sidebar( array(
            'name'          => esc_html__( 'Shop Sidebar', 'maiko' ),
            'id'            => 'sidebar-shop',
            'before_widget' => '<section id="%1$s" class="widget %2$s">',
            'after_widget'  => '</section>',
            'before_title'  => '<h2 class="widget-title"><span>',
 ...
 */
add_action( 'wp_enqueue_scripts', 'maiko_scripts' );
function maiko_scripts() {  
    $maiko_version = wp_get_theme( get_template() );
    ...
    /* Icons Lib - CSS */
    wp_enqueue_style('flaticon', get_template_directory_uri() . '/assets/fonts/flaticon/css/flaticon.css' , array(), $maiko_version->get( 'Version' ));
    
    wp_enqueue_style('flaticon', get_template_directory_uri() . '/assets/fonts/walsheim/flaticon.css' , array(), $maiko_version->get( 'Version' ));
    ...
    /* Parallax Scroll */
    wp_enqueue_script( 'pxl-parallax-background', get_template_directory_uri() . '/assets/js/libs/parallax-background.js', array( 'jquery' ), $maiko_version->get( 'Version' ), true );
    wp_enqueue_script( 'pxl-parallax-scroll', get_template_directory_uri() . '/assets/js/libs/parallax-scroll.js', array( 'jquery' ), $maiko_version->get( 'Version' ), true );
    wp_register_script( 'pxl-easing', get_template_directory_uri() . '/assets/js/libs/easing.js', array( 'jquery' ), '1.3.0', true );
    wp_enqueue_script( 'three-lib', get_template_directory_uri() . '/assets/js/libs/three.min.js', array( 'jquery' ), '1.0.0', true );
    ...
    /* Woocommerce */
    wp_enqueue_script( 'pxl-woocommerce', get_template_directory_uri() . '/woocommerce/js/woocommerce.js', array( 'jquery' ), $maiko_version->get( 'Version' ), true );

    /* Cookie */
    wp_register_script( 'pxl-cookie', get_template_directory_uri() . '/assets/js/libs/cookie.js', array( 'jquery' ), '1.4.1', true );

    /* Smooth Scroll */
    $smooth_scroll = maiko()->get_theme_opt( 'smooth_scroll', 'off' );
    if($smooth_scroll == 'on') {
        wp_enqueue_script( 'gsap' );
        wp_enqueue_script( 'pxl-scroll-trigger' );
        wp_enqueue_script( 'pxl-ScrollToPlugin' );
        wp_enqueue_script( 'pxl-bundled-lenis' );
    }
    
    wp_enqueue_script('maiko-elementor', get_template_directory_uri() . '/elements/widgets/js/elementor.js', [ 'jquery' ], $maiko_version->get( 'Version' ), true);
    wp_enqueue_script('pxl-jarallax', get_template_directory_uri() . '/assets/js/libs/jarallax.min.js', [ 'jquery' ], '1.1.0');
    wp_enqueue_style( 'pxl-caseicon', get_template_directory_uri() . '/assets/css/caseicon.css', array(), $maiko_version->get( 'Version' ) );
    wp_enqueue_style( 'pxl-grid', get_template_directory_uri() . '/assets/css/grid.css', array(), $maiko_version->get( 'Version' ) );
    wp_enqueue_style( 'pxl-style', get_template_directory_uri() . '/assets/css/style.css', array(), $maiko_version->get( 'Version' ) );
    wp_add_inline_style( 'pxl-style', maiko_inline_styles() );
    wp_enqueue_style( 'pxl-base', get_template_directory_uri() . '/style.css', array(), $maiko_version->get( 'Version' ) );
    wp_enqueue_style( 'pxl-google-fonts', maiko_fonts_url(), array(), null );
    wp_enqueue_script( 'pxl-main', get_template_directory_uri() . '/assets/js/theme.js', array( 'jquery' ), $maiko_version->get( 'Version' ), true );
    wp_localize_script( 'pxl-main', 'main_data', array( 'ajax_url' => admin_url( 'admin-ajax.php' ) ) );
    if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
        wp_enqueue_script( 'comment-reply' );
    }
    do_action( 'maiko_scripts');
}

/**
 * Enqueue Styles Scripts : Back-End
 */
add_action('admin_enqueue_scripts', 'maiko_admin_style');
function maiko_admin_style() {
    $theme = wp_get_theme( get_template() );
    wp_enqueue_style( 'maiko-admin-style', get_template_directory_uri() . '/assets/css/admin.css', array(), $theme->get( 'Version' ) );
...
add_action( 'elementor/editor/before_enqueue_scripts', function() {
    ...
    wp_enqueue_style( 'maiko-admin-style', get_template_directory_uri() . '/assets/css/admin.css');
} );

/* Favicon */
add_action('wp_head', 'maiko_site_favicon');
function maiko_site_favicon(){
    $favicon = maiko()->get_theme_opt( 'favicon' );
 ...
 */
add_action( 'wp_head', 'maiko_pingback_header' );
function maiko_pingback_header(){
...
/* Hidden Panel */
add_action( 'pxl_anchor_target', 'maiko_hook_anchor_templates_hidden_panel');
function maiko_hook_anchor_templates_hidden_panel(){

    $hidden_templates = maiko_get_templates_slug('hidden-panel');
...
if(!function_exists('maiko_hook_anchor_hidden_panel')){
    function maiko_hook_anchor_hidden_panel($args){
...
/* Elementor Popup */
add_action( 'pxl_anchor_target', 'maiko_hook_anchor_templates_popup');
function maiko_hook_anchor_templates_popup(){

    $popup_templates = maiko_get_templates_slug('popup');
...
/* Search Popup */
if(!function_exists('maiko_hook_anchor_search')){
    function maiko_hook_anchor_search(){
        $logo_s = maiko()->get_theme_opt( 'logo_s', ['url' => get_template_directory_uri().'/assets/img/logo.png', 'id' => '' ] ); 
        ...
        <div id="pxl-search-popup">
            ...
            <div class="pxl-item--conent">
                ...
                <form role="search" method="get" action="<?php echo esc_url(home_url( '/' )); ?>">
                    <input type="text" required placeholder="<?php esc_attr_e('Type Your Search Words...', 'maiko'); ?>" name="s" class="search-field" />
...
if(!function_exists('maiko_hook_anchor_popup')){
    function maiko_hook_anchor_popup($args){ ?>
...
/* Page Popup */
add_action( 'pxl_anchor_target', 'maiko_hook_anchor_templates_page_popup');
function maiko_hook_anchor_templates_page_popup(){

    $page_templates = maiko_get_templates_slug('popup');
...
if(!function_exists('maiko_hook_anchor_page_popup')){
    function maiko_hook_anchor_page_popup($args){ ?>
...
/* Cart Sidebar */
if(!function_exists('maiko_hook_anchor_cart')){
    function maiko_hook_anchor_cart(){ 
        global $woocommerce; ?>
