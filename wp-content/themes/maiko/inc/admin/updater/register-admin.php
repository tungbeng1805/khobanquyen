<?php
/**
 * The Maiko_Register initiate the theme engine
 */

if( !defined( 'ABSPATH' ) ) 
	exit; // Exit if accessed directly

class Maiko_Register {
	...
	public function __construct( $config = array(), $strings = array() ) {
		$config = wp_parse_args( $config, array(
			...
			'theme_slug'     => maiko()->get_slug(),
			'theme_name'     => maiko()->get_name(),
			'version'        => '',
	...
	function admin_error() {
		echo '<div class="error"><p>' . sprintf( wp_kses_post( esc_html__( 'The %s theme needs to be registered. %sRegister Now%s', 'maiko' ) ), maiko()->get_name(), '<a href="' . admin_url( 'admin.php?page=pxlart') . '">' , '</a>' ) . '</p></div>';
	}
	
	function admin_notice() {
		echo '<div class="notice"><p>'.esc_html__( 'Purchase code is invalid. Need a license for activation', 'maiko' ).'</p></div>';
	}
	...
	function messages($merlin = false){
		...
		if ( ! $purchase_code ){
			...
			<div class="pxl-dsb-box-head-inner">
				<h6><?php echo esc_html__( 'Register License', 'maiko' ) ?></h6>
			</div>
			<?php 
			$this->form();
			?>
			<div class="pxl-dsb-box-foot">
				<a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-" target="_blank"><?php esc_html_e( 'Can’t find your purchase code?', 'maiko' ); ?></a>
			</div>
			<?php 
		}else{  
		...
		if ( false === $license_data->success ) {
			switch ( $license_data->error ) {
				...
				case 'item_name_mismatch':
					$message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.', 'maiko' ), $this->theme_name );
				break;
				case 'license_exists':
			...
			<div class="pxl-dsb-confirmation fail">
				<h6><?php echo esc_html__( 'Active false', 'maiko' ) ?></h6>
				<p><?php echo wp_kses_post( $message ) ?> <a href="<?php echo esc_url($pxl_server_info['docs_url']) ?>" target="_blank"><?php echo esc_html__( 'our help center', 'maiko' ) ?></a> or <a href="<?php echo esc_url($pxl_server_info['support_url']) ?>" target="_blank"><?php echo esc_html__( 'submit a ticket', 'maiko' ) ?></a></p>
			</div>
			<?php $this->form(); ?>
			<div class="pxl-dsb-box-foot">
				<a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-" target="_blank"><?php esc_html_e( 'Can’t find your purchase code?', 'maiko' ); ?></a>
			...
			if ( 'valid' === $license_data->license ) {
				...
				<div class="pxl-dsb-box-head"> 
					<div class="pxl-dsb-confirmation success">
						<h6><?php echo esc_html__( 'Thanks for the verification!', 'maiko' ) ?></h6>
						<p><?php echo esc_html__( 'You can now enjoy and build great websites. Looking for help? Visit', 'maiko' ) ?> <a href="<?php echo esc_url($pxl_server_info['support_url']) ?>" target="_blank"><?php echo esc_html__( 'submit a ticket', 'maiko' ) ?></a>.</p>
					</div> 
					...
					<div class="pxl-dsb-deactive">
						<form method="POST" action="<?php echo admin_url( 'admin.php?page=pxlart' )?>">
							...
							<button class="btn button" type="submit"><?php esc_html_e( 'Remove Purchase Code', 'maiko' ) ?></button>
						</form>
					</div> 
				</div> 
				<?php 
		        ...
		        $message = esc_html__( 'Response return null.', 'maiko' ); ?>
		        <div class="pxl-dsb-confirmation fail">
		          <h6><?php echo esc_html__( 'Active false', 'maiko' ) ?></h6>
		          <p><?php echo wp_kses_post( $message ) ?> <a href="<?php echo esc_url($pxl_server_info['docs_url']) ?>" target="_blank"><?php echo esc_html__( 'our help center', 'maiko' ) ?></a> or <a href="<?php echo esc_url($pxl_server_info['support_url']) ?>" target="_blank"><?php echo esc_html__( 'submit a ticket', 'maiko' ) ?></a></p>
		        </div>
	...
	function form() {
		...
		<form action="options.php" method="post" class="pxl-dsb-register-form">
			...
			<input id="<?php echo esc_attr($this->theme_slug)?>_purchase_code" name="<?php echo esc_attr($this->theme_slug)?>_purchase_code" type="text" value="<?php echo esc_attr( $license ); ?>" placeholder="<?php esc_attr_e( 'Enter your purchase code', 'maiko' ); ?>">
			<input type="submit" class="res-purchase-code" value="<?php esc_attr_e( 'Register your purchase code', 'maiko' ) ?>">
		</form>
		<?php
	}
