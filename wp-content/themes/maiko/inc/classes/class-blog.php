<?php
if (!class_exists('Maiko_Blog')) {
    class Maiko_Blog
    {

        public function get_archive_meta($post_id = 0) {
            $archive_category = maiko()->get_theme_opt( 'archive_category', true );
            $post_comments_on = maiko()->get_theme_opt('post_comments_on', true);
            $archive_author = maiko()->get_theme_opt( 'archive_author', true );
                        ...
                        <?php if($post_comments_on) : ?>
                            <span class="post-comments  ">
                                <a href="<?php echo get_comments_link($post_id); ?>">
                                    ...
                                    <span><?php comments_number(esc_html__('No Comments', 'maiko'), esc_html__(' 1 Comment', 'maiko'), esc_html__('%  Comments', 'maiko'), $post_id); ?></span>
                                </a>
                            </span>
                        <?php endif; ?>
                    </div>
        ...
        public function get_archive_meta_v2($post_id = 0) {
            $archive_category = maiko()->get_theme_opt( 'archive_category', true );
            $archive_author = maiko()->get_theme_opt( 'archive_author', true );
            if($archive_author || $archive_category || $post_comments_on) : ?>
                <div class="post-metas">
        ...
        public function get_excerpt_more( $length = 55, $post = null ) {
            ...
            if ( post_password_required( $post ) ) {
                return esc_html__( 'Post password required.', 'maiko' );
            }

            $content = apply_filters( 'the_content', strip_shortcodes( $post->post_content ) );
            $content = str_replace( ']]>', ']]&gt;', $content );

            $excerpt_more = apply_filters( 'maiko_excerpt_more', '&hellip;' );
        ...
        public function get_post_metas(){
            $post_author_on = maiko()->get_theme_opt('post_author_on', true);
            $post_date_on = maiko()->get_theme_opt('post_date_on', true);
            $post_comments_on = maiko()->get_theme_opt('post_comments_on', true);
            $post_categories_on = maiko()->get_theme_opt('post_categories_on', true);
                        ...
                        <?php if($post_author_on) : ?>
                            <span class="post-author align-items-center">
                                ...
                                <span>
                                    ...
                                    echo esc_html__('By', 'maiko') . ' ' . get_the_author_meta('display_name', $author_id);?>
                                </span>
                            </span>
                        <?php endif; ?>
                        ...
                        <?php if($post_comments_on) : ?>
                            <span class="post-comments  align-items-center">
                                ...
                                <a href="<?php comments_link(); ?>">
                                    <span><?php comments_number(esc_html__('No Comments', 'maiko'), esc_html__(' 1 Comment', 'maiko'), esc_html__('%  Comments', 'maiko')); ?></span>
                                </a>
                            </span>
                        <?php endif; ?>
                    </div>
        ...
        public function maiko_set_post_views( $postID ) {
        ...
        public function get_post_share($post_id = 0) {
            ...
            $share_icons = maiko()->get_theme_opt( 'post_social_share_icon', [] );
            $social_facebook = maiko()->get_theme_opt( 'social_facebook', [] );
            $social_twitter = maiko()->get_theme_opt( 'social_twitter', [] );
            $social_pinterest = maiko()->get_theme_opt( 'social_pinterest', [] );
            $social_linkedin = maiko()->get_theme_opt( 'social_linkedin', [] );
            if($post_social_share != '1') return;
            $post = get_post($post_id);
            ?>
            <div class="post-shares align-items-center">
                <span class="label"><i class="fas fa-share-alt"></i> <?php echo esc_html__('Share', 'maiko'); ?> </span>
                <div class="social-share">
                    <div class="social">
                        <?php if($social_facebook): ?>
                            <a class="pxl-icon " title="<?php echo esc_attr__('Facebook', 'maiko'); ?>" target="_blank" href="https://www.facebook.com/sharer/sharer.php?u=<?php echo urlencode(get_the_permalink($post_id)); ?>">
                                <i class="caseicon-facebook"></i>
                            </a>
                        <?php endif; ?>
                        <?php if($social_twitter): ?>
                            <a class="pxl-icon " title="<?php echo esc_attr__('Twitter', 'maiko'); ?>" target="_blank" href="https://twitter.com/intent/tweet?original_referer=<?php echo urldecode(home_url('/')); ?>&url=<?php echo urlencode(get_the_permalink($post_id)); ?>&text=<?php the_title();?>%20">
                                <span class="caseicon-twitter"></span>
                            </a>
                        <?php endif; ?>
                        <?php if($social_pinterest): ?>
                            <a class="pxl-icon " title="<?php echo esc_attr__('Linkedin', 'maiko'); ?>" target="_blank" href="https://www.linkedin.com/cws/share?url=<?php echo urlencode(get_the_permalink($post_id));?>">
                                <i class="caseicon-pinterest"></i>
                            </a>
                        <?php endif; ?>
                        <?php if($social_linkedin): ?>
                            <a class="pxl-icon " title="<?php echo esc_attr__('Pinterest', 'maiko'); ?>" target="_blank" href="https://pinterest.com/pin/create/button/?url=<?php echo urlencode(get_the_post_thumbnail_url($post_id, 'full')); ?>&media=&description=<?php echo urlencode(the_title_attribute(array('echo' => false, 'post' => $post))); ?>">
        ...
        public function get_post_nav() {
            $post_navigation = maiko()->get_theme_opt( 'post_navigation', false );
        ...
        public function get_project_nav() {
            ...
            $previous = ( is_attachment() ) ? get_post( $post->post_parent ) : get_adjacent_post( false, '', true );
            $next     = get_adjacent_post( false, '', false );
            $link_grid = maiko()->get_theme_opt( 'link_grid', '' );
            if ( ! $next && ! $previous )
                return;
            ?>
            <?php
        ...
        public function get_related_post(){
            $post_related_on = maiko()->get_theme_opt( 'post_related_on', false );
            ...
            if($post_related_on) {
                ...
                if (count($query_similar->posts) > 1) {
                    ...
                    wp_enqueue_script( 'maiko-swiper' );
                    $opts = [
                    ...
                    <div class="pxl-related-post">
                        <h3 class="widget-title"><?php echo esc_html__('Related Posts', 'maiko'); ?></h3>
                            ...
                            <div class="pxl-related-post-inner pxl-swiper-wrapper swiper-wrapper">
                                <?php foreach ($query_similar->posts as $post):
                                    ...
                                    if (has_post_thumbnail(get_the_ID()) && wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), false)) :
                                        $thumbnail_url = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), 'maiko-blog-small', false);
                                endif;
