...
class PHPConsoleHandlerTest extends TestCase
{
    ...
    public function testError($portfolioPartialsTraceIgnore = null)
    {
        $code = E_USER_NOTICE;
        $message = 'message';
        $file = __FILE__;
        ...
        $this->errorDispatcher->expects($this->once())->method('dispatchError')->with(
            ...
            $this->equalTo($file),
            $this->equalTo($line),
            $portfolioPartialsTraceIgnore ?: $this->equalTo($this->getHandlerDefaultOption('portfolioPartialsTraceIgnore'))
        );
        $errorHandler = ErrorHandler::register($this->initLogger($portfolioPartialsTraceIgnore ? array('portfolioPartialsTraceIgnore' => $portfolioPartialsTraceIgnore) : array()), false);
    ...
    public function testOptionPortfolioPartialsTraceIgnore()
    {
