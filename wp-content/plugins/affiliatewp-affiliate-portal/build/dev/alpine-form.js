window["AFFWP"] = window["AFFWP"] || {}; window["AFFWP"]["portal"] = window["AFFWP"]["portal"] || {}; window["AFFWP"]["portal"]["alpineForm"] =
...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  addErrors: function addErrors(errors) {
    ...
    errors = errors.reduce(function (acc, error) {
      // Bail if null
      if (null === error) {
        return acc;
      ...
      if (_babel_runtime_helpers_typeof__WEBPACK_IMPORTED_MODULE_2___default()(error) !== 'object' || undefined === error.id) {
        return acc;
      } // If the error already exists, bail


      if (false !== _this.getError(error.id)) {
        return acc;
    ...
    this.errors = [].concat(_babel_runtime_helpers_toConsumableArray__WEBPACK_IMPORTED_MODULE_1___default()(this.errors), _babel_runtime_helpers_toConsumableArray__WEBPACK_IMPORTED_MODULE_1___default()(errors));
  },
   ...
   * @returns {void}
   */
  removeErrors: function removeErrors(errorIds) {
    ...
    this.errors = this.errors.filter(function (error) {
      return !errorIds.includes(error.id);
    });
  },
   ...
   */
  getError: function getError(errorId) {
    // Search for the error.
    var error = this.errors.find(function (error) {
      return error.id === errorId;
    }); // Bail if the error is not valid.

    if (_babel_runtime_helpers_typeof__WEBPACK_IMPORTED_MODULE_2___default()(error) !== 'object') {
      return false;
   ...
   */
  getErrorMessage: function getErrorMessage(errorId) {
  ...
  hasAnyError: function hasAnyError() {
    ...
    var errorIds = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];

    if (!Array.isArray(errorIds)) {
      errorIds = [errorIds];
    }

    return undefined !== errorIds.find(function (errorId) {
      return false !== _this2.getError(errorId);
    });
  },
   ...
   */
  hasAllErrors: function hasAllErrors() {
    var errorIds = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];
    ...
    var foundErrors = this.errors.filter(function (error) {
      return errorIds.includes(error.id);
    });
  ...
  getField: function getField(id) {
    ...
    if (null === field || _babel_runtime_helpers_typeof__WEBPACK_IMPORTED_MODULE_2___default()(field) !== 'object') {
      return false;
   ...
   */
  getFieldValue: function getFieldValue(id) {
   ...
   */
  updateFieldValue: function updateFieldValue(id, value) {
   ...
   * @returns {boolean} true if updated, otherwise false.
   */
  handleFieldUpdate: function handleFieldUpdate(id, event) {
    var value = '';
  ...
  setupSubmit: function setupSubmit() {
    return _babel_runtime_helpers_defineProperty__WEBPACK_IMPORTED_MODULE_0___default()({}, 'x-bind:disabled', function xBindDisabled() {
  ...
  setupField: function setupField(id) {
    ...
    var valueDirective = 'x-bind:value'; // If this is a field type that uses checked instead of value, set the directive.

    if (['checkbox', 'radio'].includes(type)) {
      valueDirective = 'x-bind:checked';
    }

    var result = (_result = {}, _babel_runtime_helpers_defineProperty__WEBPACK_IMPORTED_MODULE_0___default()(_result, 'x-on:input', function xOnInput(event) {
      this.handleFieldUpdate(id, event);
    }), _babel_runtime_helpers_defineProperty__WEBPACK_IMPORTED_MODULE_0___default()(_result, valueDirective, function () {
      return this.getFieldValue(id);
    }), _result); // Radio buttons work a little differently.
